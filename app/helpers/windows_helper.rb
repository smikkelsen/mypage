    ...
    window_items.in_groups_of(window.column_count) do |row|
      ...
      row.each do |item|
        ...
        unless item.nil?
          ...
          text += "              <td class='icon-col'>#{render_icon(item.icon_id, 18, styles = 'padding-right:5px;')}</div>\r\n"
          text += "              <td class='items-col'>\r\n"
          text += "                <span #{hover_text_if(item.hover_text)} class='#{hover_class_if(item.hover_text)}'>#{link_to_unless item.link.empty?, item.name, item.link rescue '' }</span>\r\n"
          text += "              </td>\r\n"
